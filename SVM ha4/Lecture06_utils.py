...
def call_classifier_chunked(classifier_pred_or_decide, X):
...
def plot_2d_separator(classifier, X, fill=False, ax=None, eps=None, alpha=1,
                      cm=ListedColormap(['#0000aa', '#ff2020']), linewidth=None, threshold=None,
                      linestyle="solid"):
    # binary?
    if eps is None:
        eps = X.std() / 2.
    ...
    if hasattr(classifier, "decision_function"):
        decision_values = call_classifier_chunked(classifier.decision_function,
                                                   X_grid)
...
def func_load_customized_dataset():   
